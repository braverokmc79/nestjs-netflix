...
interface PaginationResult {
  pagination: { order: string[]; take: number; total: number; };
...
describe('MoviesService', () => {
  ...
  describe('findAll', () => {
    ...
    it('should return a list of movies with user likes', async () => {
      ...
      jest.spyOn(commonService, 'applyCursorPaginationParamsToQb').mockReturnValue({
        pagination: {
          order: [],
          take: 0,
          total: 0,
    ...
    it('should return movies without title filter', async () => {
      ...
      jest.spyOn(commonService, 'applyCursorPaginationParamsToQb').mockResolvedValue({
        ...
        [x: string]: string | object[] | { order: string[]; take: number; total: number; } | null;
        pagination: { order: string[]; take: number; total: number; };
        nextCursor: string | null;
      });
